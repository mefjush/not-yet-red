...
export default function Tune({lightConfig}: {lightConfig: LightConfig}) {
  ...
  for (let segmentIdx = 0; segmentIdx < phases.length; segmentIdx++) {
    ...
    const duration = (segmentIdx == phases.length - 1) ? (-offset) : phases[index].duration
  ...
  const segments = cellsFiltered.map((cell, idx) => createSegment(cell.phase, cell.duration, lightConfig.cycleLength(), idx, cellsFiltered.length))
